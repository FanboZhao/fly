...
class Learner(DistributionalMPOLearner):
    ...
    def __init__(
        ...
        counter = counting.Counter(parent=RemoteAsLocal(counter), prefix=label)
...
class EnvironmentLoop(acme.EnvironmentLoop):
    ...
    def __init__(
        ...
        counter = counting.Counter(parent=RemoteAsLocal(counter), prefix=actor_or_evaluator)
        if self._config.logger is None:
            logger = loggers.make_default_logger(
                label=label,
                save_data=save_data,
    ...
    def _make_actor(
        ...
        if variable_source:
            local_variable_source = RemoteAsLocal(variable_source)

            # Create the variable client responsible for keeping the actor up-to-date.
            variable_client = variable_utils.VariableClient(
                client=local_variable_source,
